...
EXPORT CONST double xerfinv_u5(double x) {
  // Giles erfinv gems
  double t, w, p, r;
  double wc, wc2, wc4, wc8, pc, pc2, pc3;
  double ws, ws2, ws4, ws6, ps, ps2, ps3;
  double wt, wt2, wt4, wt6, pt, pt2, pt3;
  t = (1.0 - x) * (1.0 + x);
  w = - xlog_u1( t );
  ...
  if ( w < 6.250 ) {
    ...
    pc3 =  -3.6444120640178196996e-21;
    pc3 =   -1.685059138182016589e-19 + pc3 * wc;
    pc3 =   1.2858480715256400167e-18 + pc3 * wc;
    pc3 =    1.115787767802518096e-17 + pc3 * wc;
    pc3 =   -1.333171662854620906e-16 + pc3 * wc;
    pc3 =   2.0972767875968561637e-17 + pc3 * wc;
    pc3 =   6.6376381343583238325e-15 + pc3 * wc;
    
    pc2 =  -4.0545662729752068639e-14;
    pc2 =  -8.1519341976054721522e-14 + pc2 * wc;
    pc2 =   2.6335093153082322977e-12 + pc2 * wc;
    pc2 =  -1.2975133253453532498e-11 + pc2 * wc;
    pc2 =  -5.4154120542946279317e-11 + pc2 * wc;
    pc2 =    1.051212273321532285e-09 + pc2 * wc;
    pc2 =  -4.1126339803469836976e-09 + pc2 * wc;
    pc2 =  -2.9070369957882005086e-08 + pc2 * wc;

    pc =   4.2347877827932403518e-07;
    pc =  -1.3654692000834678645e-06 + pc * wc;
    pc =  -1.3882523362786468719e-05 + pc * wc;
    pc =    0.0001867342080340571352 + pc * wc;
    pc =  -0.00074070253416626697512 + pc * wc;
    pc =   -0.0060336708714301490533 + pc * wc;
    pc =      0.24015818242558961693 + pc * wc;
    pc =       1.6536545626831027356 + pc * wc;

    pc2 += pc3 * wc8;
    pc += pc2 * wc8;
  }
  else if ( w < 16.0 ) {
    ws = xsqrt_u05(w) - 3.250;
    ws2 = ws * ws;
    ws4 = ws2 * ws2;
    ws6 = ws4 * ws2;

    ps3 =   2.2137376921775787049e-09;
    ps3 =   9.0756561938885390979e-08 + ps3 * ws;
    ps3 =  -2.7517406297064545428e-07 + ps3 * ws;
    ps3 =   1.8239629214389227755e-08 + ps3 * ws;
    ps3 =   1.5027403968909827627e-06 + ps3 * ws;
    ps3 =   -4.013867526981545969e-06 + ps3 * ws;
    ps3 =   2.9234449089955446044e-06 + ps3 * ws;

    ps2 =   1.2475304481671778723e-05;
    ps2 =  -4.7318229009055733981e-05 + ps2 * ws;
    ps2 =   6.8284851459573175448e-05 + ps2 * ws;
    ps2 =   2.4031110387097893999e-05 + ps2 * ws;
    ps2 =   -0.0003550375203628474796 + ps2 * ws;
    ps2 =   0.00095328937973738049703 + ps2 * ws;

    ps =   -0.0016882755560235047313;
    ps =    0.0024914420961078508066 + ps * ws;
    ps =   -0.0037512085075692412107 + ps * ws;
    ps =     0.005370914553590063617 + ps * ws;
    ps =       1.0052589676941592334 + ps * ws;
    ps =       3.0838856104922207635 + ps * ws;
    ...
    pt3 =  -2.7109920616438573243e-11;
    pt3 =  -2.5556418169965252055e-10 + pt3 * wt;
    pt3 =   1.5076572693500548083e-09 + pt3 * wt;
    pt3 =  -3.7894654401267369937e-09 + pt3 * wt;
    pt3 =   7.6157012080783393804e-09 + pt3 * wt;
    
    pt2 =  -1.4960026627149240478e-08;
    pt2 =   2.9147953450901080826e-08 + pt2 * wt;
    pt2 =  -6.7711997758452339498e-08 + pt2 * wt;
    pt2 =   2.2900482228026654717e-07 + pt2 * wt;
    pt2 =  -9.9298272942317002539e-07 + pt2 * wt;
    pt2 =   4.5260625972231537039e-06 + pt2 * wt;

    pt =  -1.9681778105531670567e-05;
    pt =   7.5995277030017761139e-05 + pt * wt;
    pt =  -0.00021503011930044477347 + pt * wt;
    pt =  -0.00013871931833623122026 + pt * wt;
    pt =       1.0103004648645343977 + pt * wt;
    pt =       4.8499064014085844221 + pt * wt;

    pt2 += pt3 * wt6;
    pt += pt2 * wt6;
  }

  p = ( w < 6.250 ) ? pc : ( ( w < 16.0 ) ? ps : pt);

  r = (t < 0.0) ? SLEEF_NAN : p * x;
  r = (t == 0.0) ? SLEEF_INFINITY * x : r;
  r = xisnan(t) ? SLEEF_NAN : r;
  return r;
}
